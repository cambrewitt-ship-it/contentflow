...
const ALL_PLATFORMS = [...FEED_PLATFORMS, ...STORY_PLATFORMS]
...
// ─── Component ────────────────────────────────────────────────────────────────
export default function FreeSocialPreviewPage() {
  const { user } = useAuth()
    ...
    <div className="relative max-w-[240px] mx-auto rounded-2xl overflow-hidden bg-black" style={{ aspectRatio: '9/16', maxHeight: '420px' }}>
      ...
      {isAdvert && (
        <div className="absolute top-9 left-3">
          <span className="bg-white/20 backdrop-blur-sm text-white text-[10px] font-medium px-2 py-0.5 rounded-full border border-white/30">
            Sponsored
          </span>
        </div>
      )}
      ...
      <div className="absolute bottom-0 left-0 right-12 p-3 space-y-1.5">
        ...
        {isAdvert && (
          <div className="flex items-center gap-2 pt-1">
            ...
            <button className="flex-shrink-0 bg-[#FE2C55] text-white text-xs font-bold px-3 py-1.5 rounded-md">
              {ctaText}
            </button>
          </div>
        )}
    ...
    <div className="relative max-w-[240px] mx-auto rounded-2xl overflow-hidden bg-black" style={{ aspectRatio: '9/16', maxHeight: '420px' }}>
      ...
      {isAdvert && (
        <div className="absolute bottom-14 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1">
          ...
          <button className="bg-white text-gray-900 text-xs font-bold px-5 py-2 rounded-full shadow-lg">
            {ctaText}
          </button>
        </div>
      )}
  ...
  const renderIGStoriesPreview = () => (
    <div className="relative max-w-[240px] mx-auto rounded-2xl overflow-hidden bg-black" style={{ aspectRatio: '9/16', maxHeight: '420px' }}>
      ...
      {/* Advert CTA – swipe-up style */}
      {isAdvert ? (
        <div className="absolute bottom-3 left-3 right-3 flex flex-col items-center gap-1.5">
          {headline && <p className="text-white text-xs font-semibold text-center">{headline}</p>}
          <button className="w-full bg-white text-gray-900 text-xs font-bold py-2.5 rounded-xl shadow-lg">
            {ctaText}
          </button>
        </div>
      ) : (
        /* Organic – reply + emoji row */
        <div className="absolute bottom-3 left-3 right-3 flex items-center gap-2">
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border/40 bg-muted/30">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <h1 className="text-3xl font-bold tracking-tight">Free Social Media Preview</h1>
          <p className="mt-2 text-muted-foreground">See exactly how your post will look — organic or as a paid ad. No account needed.</p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="space-y-4">
                ...
                {postImage ? (
                  <div className="relative rounded-lg overflow-hidden border border-border bg-muted/20">
                    ...
                    <button onClick={() => imageInputRef.current?.click()}
                      className="absolute bottom-2 right-2 px-3 py-1.5 bg-black/60 text-white text-xs rounded-lg hover:bg-black/80 transition-colors">
                      Change
                    </button>
                  </div>
                ) : (
            ...
            {/* ── Advert / Organic toggle + ad fields ── */}
            <div className="bg-card border border-border rounded-xl p-5 space-y-4">
                ...
                <div className="space-y-3 pt-1">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      placeholder="e.g. Try Our New Summer Blend"
                      className="w-full px-3 py-2 text-sm border border-input rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-ring placeholder:text-muted-foreground"
                    />
                  </div>
                      ...
                      {ctaOpen && (
                        <div className="absolute z-10 top-full left-0 right-0 mt-1 bg-popover border border-border rounded-lg shadow-lg overflow-hidden">
                          {CTA_OPTIONS.map((opt) => (
                            <button
                              key={opt}
                              onClick={() => { setCtaText(opt); setCtaOpen(false) }}

...
import { Loader2, AlertCircle, CheckCircle, XCircle, AlertTriangle, Minus, Calendar, Grid3X3 } from 'lucide-react';
...
export default function PortalApprovalsPage() {
  ...
  // Flatten posts for month view
  const allPosts = useMemo(() => {
    return weeks.flatMap(week => week.posts).map(post => ({
      id: post.id,
  ...
  const handleDateClick = (date: Date) => {
    console.log('📅 Date clicked in month view:', date);
    setViewMode('week');
    // The week view will automatically show the week containing the clicked date
  };
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">
                Review and approve your scheduled content organized by {viewMode === 'week' ? 'week' : 'month'}
                {client && ` • ${client.name}`}
              </p>
            </div>
